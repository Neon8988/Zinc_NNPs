...
class BaseTrainer(ABC):
    ...
    def __init__(
        ...
        self.config = {
            ...
            "model": model_copy.pop("name"),
            "model_attributes": model_copy,
            "optim": optimizer,
            "logger": logger,
            "amp": amp,
    ...
    def load_datasets(self):
        ...
        if self.config.get("dataset", None):
            ...
            if self.config.get("val_dataset", None):
                ...
                subsampling_range = int(
                    len(self.val_dataset)
                    * self.config["val_dataset"].get(
                        "subsampling_fraction", 1.0
                    )
                )
                subsampling_index = np.random.RandomState(seed=42).permutation(
                    len(self.val_dataset)
                )[:subsampling_range]
                self.val_dataset_sub = Subset(
                    self.val_dataset, subsampling_index
                )
                ...
                self.val_sampler_sub = self.get_sampler(
                    self.val_dataset_sub,
                    self.config["optim"].get(
                        "eval_batch_size", self.config["optim"]["batch_size"]
                    ),
                    shuffle=False,
                ...
                self.val_loader_sub = self.get_dataloader(
                    self.val_dataset_sub,
                    self.val_sampler_sub,
                )
    ...
    def save_results(self, predictions, results_file, keys):
        ...
        if distutils.is_master():
            ...
            # with open('/mnt/gs21/scratch/jinhongn/pyg/own_EwaldMP/zinc/save_force.pkl', 'wb') as f:
            #     pickle.dump(gather_results, f)
            for k in keys:
                gather_results[k] = np.array(gather_results[k], dtype=object)
            #     if k == "forces":
