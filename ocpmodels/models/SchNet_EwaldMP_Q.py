...
@registry.register_model("SchNet_EwaldMP_Q")
class SchNetWrap(SchNet, BaseModel):
    ...
    def _forward(self, data):
        ...
        h_ewald=h_z+d_embed.unsqueeze(-1).expand(h.shape[0],h.shape[1])/h.shape[1]
        if self.residual:
            h_ewald=self.resblock(h_ewald)
        else:
            h_ewald=h_ewald
        #h_ewald=self.resblock(h_ewald)
        # scal_charge_total=torch.sum(h_ewald,dim=1)
        # scal_pred_charge=scal_charge_total.new_zeros(batch_size).index_add_(0, batch, scal_charge_total)
        # #print(f'=========E:{E}')
        # print(f'=========pred_embed:{scal_pred_charge}')
        ...
        if self.use_ewald :
            ...
            for i in range(self.num_interactions):
                ...
                dh_ewald, dot, sinc_damping = self.ewald_blocks[i](h_ewald,pos,k_grid,batch_size,batch,dot,sinc_damping)
                h_ewald =h_ewald+dh_ewald
                h_t=self.skip_connection_factor * (h+h_ewald)
            if self.seperated:
                hzs=torch.nn.Sequential(self.lin1,self.act,self.lin2)(h)
                h_e=torch.nn.Sequential(self.lin1,self.act,self.lin2)(h_ewald)
